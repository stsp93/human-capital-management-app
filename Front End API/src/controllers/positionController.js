...
router.get('/', async (req, res) => {
    try {
        ...
        const employees = await Promise.all(positions.results.map(pos => employeeService.getById(pos.employeeId._id, req.token)))
        positions.results.forEach((pos, i) => {
            pos.employee = employees[i];
        });

        attachPaginationHrefs(positions, req.query)
        res.render('positionsList', { positions, department });
    } catch (error) {
        console.log(error);
        res.render('positionsList', { error });
    }
